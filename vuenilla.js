...
(function(exports) {
    ...
    */
    const interpolate = function(_this, s, params, _tag) {
        const tag = _tag || ''
        return new_function(_this, params, `return ${tag}\`${s}\`;`, {
            is_dyn_tag
        });
    }
    ...
    */
    function get_proxy($el, _, _parent) {
        ...
        if (!_parent) {
            ...
            res.$directives = Object.assign({}, {
                'v-for': vFor
            }, _.$directives || {}); //overwrite with $directive if one passed it
    ...
    */
    function next_sibiling(sibiling) {
        if (sibiling != null) {
            while (sibiling.nextSibling.nodeType == 3) {
        ...
        for (const node of parent_node.childNodes) {
            if (node.nodeType == 3) { //text element
                ...
                if (is_conditional(node)) {
                    ...
                    _.$reactive_list.push({
                        f: reactive(attr, _, node, node, sibiling, next_element)
                    });
                    node.remove();
                } else {
                    sibiling = node;
    ...
    */
    ...
    function vModel($el, attr, _) {
        ...
        if ($el.tagName == 'INPUT' || $el.tagName == 'TEXTAREA' || $el.tagName == 'SELECT') {
            ...
            _.$reactive_list.push({
                f: () => setAttribute($el, value, _[model])
            })
        ...
        for (const attr of $el.attributes) {
            ...
            if (attr.name == ':style') vStyle($el, attr, _parent)
                ...
                if (is_dyn(attr.value, _parent)) {
                    _parent.$reactive_list.push({
                        text: attr.value,
                        f: (text) => setAttribute($el, attr.name, text)
                    });
                }
            }
            if (remove_attribute) {
